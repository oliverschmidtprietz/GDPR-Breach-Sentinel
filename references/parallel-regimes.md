...
**Applicability:** Art. 73 obligations apply from **2 August 2026**, but the underlying high-risk obligations are AI-Omnibus-postponed: Annex III systems to **2 December 2027**, and high-risk systems that are products (or safety components) under the Annex I sectoral legislation to **2 August 2028**. Art. 73(9)–(10) defer to equivalent sectoral incident-reporting regimes (e.g., MDR/IVDR for medical devices) for some categories — for those, only limb (c) fundamental-rights incidents are reported under the AI Act. **Before the applicable date, present the duty as "applies from", never as live.** Canonical date source: the EU-AI-Act-Suite timeline (`ai-act-high-risk/references/ai-omnibus-timeline-postponements.md`).
